...
export const Comparer = {
	async compareGuildLegendWeekProgress(freshGuild: any): Promise<ILegendPlayerProgress[]> {
		...
		freshGuild.forEach((freshMember: any) => {
			let oldMember = oldGuild.find(
				(oldMember: any) => oldMember.player_name === freshMember.player_name
			);
			let kDiff =
